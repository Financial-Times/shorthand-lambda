...
describe("main.js", () => {
  beforeEach(() => {
    ...
    fixtureEventPage = JSON.parse(
      readFileSync(
        resolve(__dirname, "..", "test-fixtures/test-event.json")
      ).toString()
    );
    fixtureEventAsset = JSON.parse(
      readFileSync(
  ...
  const resultStringBase =
    "Deployed to: http://dest-bucket.s3-website-test-region.amazonaws.com/";
    ...
    it("processes HTML with UUID", (done) => {
      ...
      main(fixtureEventPage, {}, (err, result) => {
        ...
        expect(result).to.equal(
          resultStringBase + "test-fixture-path/index.html"
        );
        done();
      });
    });
  ...
  describe("processing assets", () => {
    ...
    it("calls back with result URL", (done) => {
      main(fixtureEventAsset, {}, (err, result) => {
        expect(err).not.to.exist;
        expect(result).to.equal(
          resultStringBase + "test-fixture-path/index.jpg"
        );
        done();
      });
    });
